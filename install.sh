...
read -p "What do you want to install? (rocketchat, mattermost): " app

if [[ $app == "rocketchat" ]]; then
    echo "Installing rocketchat..."
    echo "choose installation method (Snap, RocketChatCTL): "
    read method
    if [[ $method == "Snap" ]]; then
        echo "Installing Snap..."
        sudo snap install rocketchat-server
        echo "Done!"
    elif [[ $method == "RocketChatCTL" ]]; then
        echo "Installing RocketChatCTL..."
        bash -c "$(curl https://install.rocket.chat)"
        echo "Done!"
    else
        echo "Invalid method!"
    fi
elif [[ $app == "mattermost" ]]; then
